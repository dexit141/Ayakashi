    ...
    protected void renderLeash(T entityLivingIn, double x, double y, double z, float entityYaw, float partialTicks) {
        if (!Config.isShaders() || !Shaders.isShadowPass) {
            ...
            if (entity != null) {
                ...
                double d0 = this.interpolateValue(entity.prevRotationYaw, entity.rotationYaw, partialTicks * 0.5F) * (double) ((float) Math.PI / 180F);
                double d1 = this.interpolateValue(entity.prevRotationPitch, entity.rotationPitch, partialTicks * 0.5F) * (double) ((float) Math.PI / 180F);
                ...
                double d9 = this.interpolateValue(entityLivingIn.prevRenderYawOffset, entityLivingIn.renderYawOffset, partialTicks) * (double) ((float) Math.PI / 180F) + (Math.PI / 2D);
